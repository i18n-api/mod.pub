...
use aok::Result;
use intbin::u64_bin;
use r::fred::interfaces::{HashesInterface, SetsInterface};
...
pub const MAX_DEBT: i64 = -100 * 100;
...
async fn _bill_new(uid: u64, cid: u16, kid: u64, rid: u64, amount: i64) -> Result<i64> {
  ...
  () = r::R
    .hset(K::PAY_COST, (u64_bin(uid), &reamin.to_le_bytes()[..]))
    .await?;

  Ok(reamin)
}

pub async fn remain(uid: u64) -> Result<i64> {
  let r: Option<Vec<u8>> = r::R.hget(K::PAY_COST, u64_bin(uid)).await?;
  if let Some(r) = r {
...
pub async fn cost_cent(uid: u64, cid: u16, kid: u64, rid: u64, amount: u64) -> Result<i64> {
  ...
  let remain = _bill_new(uid, cid, kid, rid, -amount).await?;
...
pub async fn topup_cent(uid: u64, cid: u16, kid: u64, rid: u64, amount: u64) -> Result<i64> {
  let amount = amount as i64;
  let remain = _bill_new(uid, cid, kid, rid, amount as i64).await?;
  if remain >= MAX_DEBT {
